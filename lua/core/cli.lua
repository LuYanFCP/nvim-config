...
function cli:env_init()
    ...
    package.path = package.path .. ';' .. self.rtp .. '/lua/vim/?.lua;' ..
                       self.module_path .. '/?.lua;'
    local shared = assert(loadfile(helper.path_join(self.rtp, 'lua', 'vim',
                                                    'shared.lua')))
    _G.vim = shared()
end
        ...
        local check_lazy = function(t, data)
            vim.tbl_filter(function(k)
                if vim.tbl_contains(lazy_keyword, k) then
                    data.load = type(t[k]) == 'table' and
                                    table.concat(t[k], ',') or t[k]
                    return true
                end
                return false
            end, vim.tbl_keys(t))
...
function cli:boot_strap()
    ...
    helper.run_git('lazy.nvim',
                   'git clone https://github.com/folke/lazy.nvim ' ..
                       self.lazy_dir, 'Install')
...
function cli:installer(type)
    ...
    for name, v in pairs(packages or {}) do
        if v.type:find('Remote') then
            ...
            if helper.isdir(path) and type == 'install' then
                ...
                local cmd = type == 'install' and url .. name .. ' ' .. path or
                                'git -C ' .. path .. ' pull'
    ...
    if not vim.tbl_contains(res, true) then
        helper.green(
            '🎉 Congratulations Config install or update success. Enjoy ^^')
        return
    end
    helper.red(
        'Some plugins not install or update success please run install again')
end

function cli.install() cli:installer('install') end

function cli.update() cli:installer('update') end

function cli.clean() os.execute('rm -rf ' .. cli.lazy_dir) end
...
function cli.modules()
    ...
    for dict in p:lines() do
        ...
        if dict[#dict] ~= 'modules' then table.insert(res, dict[#dict]) end
    end
...
function cli:meta(arg) return function(data) self[arg](data) end end
